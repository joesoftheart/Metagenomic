...
$this->myfpdf->Cell(0, 8, 'Program analysis : ' . "Qiime2", 0, 1);
...
$this->myfpdf->MultiCell(0,6,'The data set '.$project_name.' was uploaded on '.$day.' at '.$time.' . Raw sequencing data in fasta format were implemented according to QIIME ver. 2 (https://docs.qiime2.org/2018.8/) based on VSEARCH. First, raw sequences of each sample were merged using python script (in-house script). After that merged fasta sequence was removed the replication with the dereplicate-sequences command. The clustering of FeatureTable [Frequency] and FeatureData [Sequence] to make an OTU using '.$precent_identity.'. '.$textmes.'The chimeras were also screened using VSEARCH function and removed from the sequences set. The clean sequences were generated a tree for phylogenetic diversity analyses. The OTUs was subsampled based on the lowest count sequences for alpha and beta analysis using core-metrics-phylogenetic method. Alpha diversity metrics, including rarefaction curves and a measure of community richness and evenness metrics were visualized using qiime diversity command which was the testing for associations between categorical metadata column and alpha diversity data. Beta diversity analysis, including bray_curtis and Jaccard index were calculated using the beta-group-significance command in Qiime2. For taxonomy analysis, a pre-trained Naïve Bayes classifier and the q2-feature-classifier were applied in taxonomy assignment.');


$this->myfpdf->Cell(0, 5, '', 0, 1);
$this->myfpdf->SetFont('Times', '', 10);
$this->myfpdf->MultiCell(0, 10, $this->myfpdf->WriteHTML('<b>Table 1</b> The summary of the number of samples after data processing step.') . '', 0);
...
$this->myfpdf->SetFont('Times', '', 10);
$this->myfpdf->MultiCell(0, 6, 'A total of '.$project_name.'  datasets has been submitted to the Amplicon Metagenomic platform. These data were then analysed and display as different diagrams/graphs. This report consists of statistical table and graph of alpha diversity analysis, rarefaction graph, taxonomy profiles of bacterial phyla, heatmap profiling, beta diversity analysis including statistical comparison and PCoA, respectively.');
...
$this->myfpdf->Ln(2);
$this->myfpdf->SetFont('Times', '', 10);
$this->myfpdf->MultiCell(0, 6, 'After data processing, the total of number of samples is '.$project_group_sam.' samples with '.$result_sample.' in total frequency. The alpha diversity was estimated microbial diversity (Shannon) from subsampling data based on the library size at '.$subsample.'. The Shannon index estimated the diversity in the community. It displayed that there is the most diverse of bacteria in xxx and the lowest diverse of bacteria in xxx.');

$this->myfpdf->Ln(4);
$this->myfpdf->SetFont('Times', '', 10);
$this->myfpdf->MultiCell(0, 10, $this->myfpdf->WriteHTML('<b>Table 2 </b>Alpha diversity estimator of bacterial 16S analysis at '.$precent_identity.'% identity threshold') . '');
...
// $this->myfpdf->Image(base_url() .'data_report_qiime/'.$user.'/'.$project_name.'/alpha_diversity_analysis/boxplots_shannon.png', '110', '140', '70  ', '70', 'PNG');
...
// $this->myfpdf->Image(base_url() .'data_report_qiime/'.$user.'/'.$project_name.'/alpha_diversity_analysis/Rarefactionqiime.png', '50', '60', '100  ', '60', 'PNG');
...
// $this->myfpdf->Image(base_url() . 'data_report_qiime/'.$user.'/'.$project_name.'/taxonomy_classification/bar_plot.png', '20', '160', '100  ', '90', 'PNG');
// $this->myfpdf->Image(base_url() . 'data_report_qiime/'.$user.'/'.$project_name.'/taxonomy_classification/bar_plot_legend.png', '120', '170', '100  ', '60', 'PNG');
...
// $this->myfpdf->Image(base_url() .'data_report_qiime/'.$user.'/'.$project_name.'/taxonomy_classification/heatmap.png', '50', '60', '90  ', '95', 'PNG');
$this->myfpdf->SetFont('Times', 'B', 10);
...
// $this->myfpdf->Image(base_url() . 'data_report_qiime/'.$user.'/'.$project_name.'/beta_diversity_analysis/PC1_vs_PC2_plot.png', '20', '60', '60  ', '60', 'PNG');
// $this->myfpdf->Image(base_url() . 'data_report_qiime/'.$user.'/'.$project_name.'/beta_diversity_analysis/PC1_vs_PC3_plot.png', '80', '60', '60  ', '60', 'PNG');
// $this->myfpdf->Image(base_url() . 'data_report_qiime/'.$user.'/'.$project_name.'/beta_diversity_analysis/PC3_vs_PC2_plot.png', '140', '60', '60  ', '60', 'PNG');
...
//if($my_result != "off"){
    ...
    //$this->myfpdf->Image(base_url() .'data_report_qiime/'.$user.'/'.$project_name.'/optional_output/bar_plot_STAMP.png', 20, 80, 170, 90);
    $this->myfpdf->SetFont('Times', '', 10);
    $this->myfpdf->Cell('0', 110, '', 0, 1);
...
$this->myfpdf->MultiCell(0, 7, '1. Pruesse E, Quast C, Knittel K, Fuchs BM, Ludwig WG, Peplies J, Glöckner FO. SILVA: a comprehensive online resource for quality checked and aligned ribosomal RNA sequence data compatible with ARB. Nucl. Acids Res. 2007;35:7188-7196');


$this->myfpdf->MultiCell(0,7,'2. Caporaso, J. Gregory, et al. QIIME allows analysis of high-throughput community sequencing data. Nature methods 2010;7.5: 335-336.');


$this->myfpdf->MultiCell(0,7,'3. J. Gregory Caporaso, Kyle Bittinger, Frederic D. Bushman, Todd Z. DeSantis, Gary L. Andersen, and Rob Knight. PyNAST: a flexible tool for aligning sequences to a template alignment. Bioinformatics 2010;26: 266-267.');

$this->myfpdf->MultiCell(0,7,'4. Lozupone, Catherine, et al. UniFrac: an effective distance metric for microbial community comparison. The ISME journal 2011;5.2: 169.');
...
$this->myfpdf->MultiCell(0,7,'9. Jiajie Zhang, Kassian Kobert, Tomáš Flouri, Alexandros Stamatakis. PEAR: a fast and accurate Illumina Paired-End reAd mergeR. Bioinformatics. 2014; 30(5): 614–620.');


$this->myfpdf->MultiCell(0,7,'10. Parks DH, Tyson GW, Hugenholtz P, Beiko RG. STAMP: Statistical analysis of taxonomic and functional profiles. Bioinformatics, 2014;30: 3123-3124.');


$this->myfpdf->MultiCell(0,7,'11. H. Wickham. ggplot2: Elegant Graphics for Data Analysis. Springer-Verlag New York, 2016.');

$this->myfpdf->MultiCell(0,7,'12. Torbjørn Rognes, Tomáš Flouri, Ben Nichols, Christopher Quince, Frédéric Mahé.  VSEARCH: a versatile open source tool for metagenomics. PeerJ. 2016; 4: e2584. ');
