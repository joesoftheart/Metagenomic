...
Class Qiime2_report extends CI_Controller{
    ...
    public function getNameDir($path,$data_path){
    ...
    public function testdir(){
         ...
         $array_files = array();
         $array_files[0][0] = "Samples";
         $array_files[0][1] = "No.of sequences after dereplicate sequences";
         $array_files[0][2] = "No. of sequences after removing chimera";


         $p1sequence = "data_report_qiime2/$user/$project/outHTML/1_sequence/dereplicate_table/";
         $data_path1sequence = array(".","..");
         $dir_1 = $this->getNameDir($p1sequence,$data_path1sequence);
         $path2_1sequence = $p1sequence.$dir_1."/data/sample-frequency-detail.csv";

         $count = 1;
         $read = fopen($path2_1sequence,"r") or die ("Unable to open file");
         while(($line = fgets($read)) !== false){
                 list($sample,$value1) = explode(",", $line);
                 $value2 = $this->folder_2outtable($user,$project,$sample);
                
                 $array_files[$count][0] = trim($sample);
                 $array_files[$count][1] = trim($value1);
                 $array_files[$count][2] = trim($value2);

                  //echo $sample."  ".$value1."  ".$value2."<br>";
       ...
       ########################################
        ...
        $table_data = array();
        foreach ($array_files as $variable) {
            $colum = 0;
         ...
         $path_log = "data_report_qiime2/$user/$project/file_report/table_log.txt";
         $file_log = FCPATH.$path_log;
         file_put_contents($file_log,$table_data); 
  
    }

    public function folder_2outtable($user,$project,$sample){

         $p2out_table = "data_report_qiime2/$user/$project/outHTML/2_otuTable/otu_table/";
         $data_2out_table = array(".","..");
         $dir_2out_table = $this->getNameDir($p2out_table,$data_2out_table);
         $path2_2out_table = $p2out_table.$dir_2out_table."/data/sample-frequency-detail.csv";

        $read2 = fopen($path2_2out_table,"r") or die ("Unable to open file");
        while(($line2 = fgets($read2)) !== false){
               list($name,$value) = explode(",", $line2);
                 if($sample == $name){
                    return $value;
                 }
        }
    ...
    public function result_2outtable($user,$project){
    ...
    public function check_dirzip(){
        ...
        $array_status = $this->mongo_db->get_where('status_process',array('project_id' => $id_project));
         ...
         $path_img = FCPATH."data_report_qiime2/$user/$folder/Download/";  
    ...
    public function down_zip(){
         ...
         $this->zip->read_dir("data_report_qiime2/".$user."/".$folder."/Download/",FALSE);
    ...
    public function graph_qiime_full($id_project){
        ...
        $data['path_index'] = "data_report_qiime2/$user/$projects_name/outHTML/index.html";

        $this->load->view('qiime2_full_graph',$data);
    ...
    // }
     ...
     public function test_report($id_project){
        ...
        $array_status = $this->mongo_db->get_where('status_process',array('project_id' => $id_project));
         foreach ($array_status as $r) {             
              $pick_otus = $r['pick_otus'];
              $precent_identity = $r['precent_identity']; 
              $subsample = $r['subsample'];
         }

        if($pick_otus != "Denovoclustering"){
          $textmes = "The clustering is performed at ".$precent_identity."% identity against the Greengenes 13_8  ".$precent_identity."% OTUs reference database.";
        }else{
             $textmes = "The clustering is performed at ".$precent_identity."% identity to create  ".$precent_identity."% OTUs.";
        }

        $data['precent_identity']= $precent_identity;
        $data['textmes'] = $textmes;
        $data['tablelog'] = "data_report_qiime2/$user/$projects_name/file_report/table_log.txt";

        $data['result_sample'] = $this->result_2outtable($user,$projects_name);
        $data['subsample'] = $subsample;

        $this->load->library('myfpdf');
        $this->load->library('mytcpdf');
        $this->load->view('qiime2_report',$data);
