...
class Barnard(AbstractStatsTestPlugin):
  '''
  Perform Barnard's test.
  '''
  ...
  def hypothesisTest(self, seq1, seq2, totalSeq1, totalSeq2):
    ...
    for dummy in xrange(0,steps):
      ...
      for table in extremeTables:
        ...
        nCr1 = mpmath.exp(self.logChoose(totalSeq1,c1))
        nCr2 = mpmath.exp(self.logChoose(totalSeq2,c2)) 
        
        prob = nCr1 * nCr2 * mpmath.power(pi,c1+c2) * mpmath.power(1.0 - pi,totalSeq1+totalSeq2-c1-c2)
        pValue += prob
