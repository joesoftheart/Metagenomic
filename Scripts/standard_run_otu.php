...
function phylotype_count($user,$id, $project,$path){
    ...
    $cmd ="dist.seqs(fasta=final.fasta, cutoff=0.21, processors=8,inputdir=$path,outputdir=$path)
cluster(column=final.dist, count=final.count_table, method=furthest, cutoff=0.21,inputdir=$path,outputdir=$path)
make.shared(list=final.fn.unique_list.list, count=final.count_table, label=0.03-0.05-0.10-0.20,inputdir=$path,outputdir=$path)
classify.otu(list=final.opti_mcc.list, count=final.count_table, taxonomy=final.taxonomy, label=0.03,inputdir=$path,outputdir=$path)
count.groups(shared=final.opti_mcc.shared,inputdir=$path,outputdir=$path)";
    file_put_contents('owncloud/data/'.$user.'/files/'.$project.'/data/input/run.batch', $cmd);
    $cmd = "qsub -N '$jobname' -o Logs_sge -e Logs_sge -cwd -b y Mothur/mothur ../owncloud/data/$user/files/$project/data/input/run.batch ";
    exec($cmd);
    $check_qstat = "qstat  -j '$jobname' ";
...
function read_log_sungrid_phylotype_count($user,$id,$project,$path,$id_job){
    ...
    $searchfor = 'contains';
    $file = file_get_contents('Logs_sge/'.$file_name);
    $cmd = "'more' $file_name '| grep 'Total seqs:'";
...
// search, and store all matching occurences in $matches
    if(preg_match_all($pattern, $file, $matches)){
        echo "Found matches:\n";
        echo implode("\n", $matches[0]);
        $total1 =  explode(" ",$matches[0][0]);
        $total2 =  explode(" ",$matches[0][1]);
        $total3 =  explode(" ",$matches[0][2]);
        $total4 =  explode(" ",$matches[0][3]);

        $total1 = explode(".",$total1[2]);
        $total2 = explode(".",$total2[2]);
        $total3 = explode(".",$total3[2]);
        $total4 = explode(".",$total4[2]);
        echo "//////////////////////";
        $total =  min($total1[0], $total2[0], $total3[0], $total4[0]);
...
function sub_sample_sammary($user,$id, $project,$path,$total){
    ...
    $cmd ="sub.sample(shared=final.opti_mcc.shared, size=10000,inputdir=$path,outputdir=$path)
collect.single(shared=final.opti_mcc.shared, calc=chao, freq=100, label=0.03,inputdir=$path,outputdir=$path)
rarefaction.single(shared=final.opti_mcc.shared, calc=sobs, freq=100, label=0.03, processors=8,inputdir=$path,outputdir=$path)
summary.single(shared=final.opti_mcc.shared, calc=nseqs-coverage-sobs-invsimpson-chao-shannon-npshannon-simpson, subsample=$total, label=0.03,inputdir=$path,outputdir=$path)";
    file_put_contents('owncloud/data/'.$user.'/files/'.$project.'/data/input/run.batch', $cmd);
    $cmd = "qsub -N '$jobname' -o Logs_sge -e Logs_sge -cwd -b y Mothur/mothur ../owncloud/data/$user/files/$project/data/input/run.batch ";
    exec($cmd);
    $check_qstat = "qstat  -j '$jobname' ";
...
function read_name_sample($user,$id, $project,$path,$total){
    $group_sample = array();
    $name_sample = null;
    if ($file = fopen('../owncloud/data/'.$user.'/files/'.$project.'/data/input/stability.files', "r")) {
        $i = 0;
        while(!feof($file)) {
            $line = fgets($file);
...
function plot_graph($user,$id, $project,$path,$total,$name_sample)
{
    ...
    $cmd = "heatmap.bin(shared=final.opti_mcc.0.03.subsample.shared, scale=log2, numotu=50,inputdir=$path,outputdir=$path)
dist.shared(shared=final.opti_mcc.shared, calc=lennon-jclass-morisitahorn-sorabund-thetan-thetayc-braycurtis, subsample=$total, processors=8,inputdir=$path,outputdir=$path)
heatmap.sim(phylip=final.opti_mcc.thetayc.0.03.lt.ave.dist,inputdir=$path,outputdir=$path)
heatmap.sim(phylip=final.opti_mcc.jclass.0.03.lt.ave.dist,inputdir=$path,outputdir=$path)
summary.shared(calc=lennon-jclass-morisitahorn-sorabund-thetan-thetayc-braycurtis, groups=$name_sample, all=T,inputdir=$path,outputdir=$path)
venn(shared=final.opti_mcc.0.03.subsample.shared, groups=$name_sample,inputdir=$path,outputdir=$path)
venn(shared=final.opti_mcc.0.03.subsample.shared, groups=$name_sample,inputdir=$path,outputdir=$path)
tree.shared(phylip=final.opti_mcc.thetayc.0.03.lt.ave.dist, processors=8,inputdir=$path,outputdir=$path)
tree.shared(phylip=final.opti_mcc.morisitahorn.0.03.lt.ave.dist,inputdir=$path,outputdir=$path)
tree.shared(phylip=final.opti_mcc.jclass.0.03.lt.ave.dist,inputdir=$path,outputdir=$path)
tree.shared(phylip=final.opti_mcc.braycurtis.0.03.lt.ave.dist,inputdir=$path,outputdir=$path)
tree.shared(phylip=final.opti_mcc.lennon.0.03.lt.ave.dist,inputdir=$path,outputdir=$path)
parsimony(tree=final.opti_mcc.thetayc.0.03.lt.ave.tre, group=soil.design, groups=all,inputdir=$path,outputdir=$path)
unifrac.weighted(tree=final.opti_mcc.thetayc.0.03.lt.ave.tre, group=soil.design, random=T,inputdir=$path,outputdir=$path)
unifrac.unweighted(tree=final.opti_mcc.thetayc.0.03.lt.ave.tre, group=soil.design, random=T, groups=all,inputdir=$path,outputdir=$path)
pcoa(phylip=final.opti_mcc.thetayc.0.03.lt.ave.dist,inputdir=$path,outputdir=$path)
nmds(phylip=final.opti_mcc.thetayc.0.03.lt.ave.dist, mindim=3, maxdim=3,inputdir=$path,outputdir=$path)
amova(phylip=final.opti_mcc.thetayc.0.03.lt.ave.dist, design=soil.design,inputdir=$path,outputdir=$path)
homova(phylip=final.opti_mcc.thetayc.0.03.lt.ave.dist, design=soil.design,inputdir=$path,outputdir=$path)
corr.axes(axes=final.opti_mcc.thetayc.0.03.lt.ave.pcoa.axes, shared=final.opti_mcc.0.03.subsample.shared, method=spearman, numaxes=3, label=0.03,inputdir=$path,outputdir=$path)
system(mv final.opti_mcc.0.03.subsample.spearman.corr.axes final.opti_mcc.0.03.subsample.spearman.corr.axesThetayc3D,inputdir=$path,outputdir=$path)
corr.axes(axes=final.opti_mcc.thetayc.0.03.lt.ave.pcoa.axes, metadata=soilpro.metadata, method=pearson, numaxes=3, label=0.03,inputdir=$path,outputdir=$path)";
    file_put_contents('owncloud/data/' . $user . '/files/' . $project . '/data/input/run.batch', $cmd);
    $cmd = "qsub -N '$jobname' -o Logs_sge -e Logs_sge -cwd -b y Mothur/mothur ../owncloud/data/$user/files/$project/data/input/run.batch ";
    exec($cmd);
    $check_qstat = "qstat  -j '$jobname' ";
